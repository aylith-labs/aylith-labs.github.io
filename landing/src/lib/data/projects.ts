export const projectMeta = [
	{
		slug: 'bract',
		name: 'Bract',
		iconPath: 'M6.75 7.5l3 2.25-3 2.25m4.5 0h3m-9 8.25h13.5A2.25 2.25 0 0021 18V6a2.25 2.25 0 00-2.25-2.25H5.25A2.25 2.25 0 003 6v12a2.25 2.25 0 002.25 2.25z',
		gradientFrom: '#3f3f46',
		gradientTo: '#71717a',
		category: 'ai-infrastructure' as const,
		featured: true
	},
	{
		slug: 'compokit',
		name: 'Compokit',
		iconPath: 'M9.53 16.122a3 3 0 00-5.78 1.128 2.25 2.25 0 01-2.4 2.245 4.5 4.5 0 008.4-2.245c0-.399-.078-.78-.22-1.128zm0 0a15.998 15.998 0 003.388-1.62m-5.043-.025a15.994 15.994 0 011.622-3.395m3.42 3.42a15.995 15.995 0 004.764-4.648l3.876-5.814a1.151 1.151 0 00-1.597-1.597L14.146 6.32a15.996 15.996 0 00-4.649 4.763m3.42 3.42a6.776 6.776 0 00-3.42-3.42',
		gradientFrom: '#ec4899',
		gradientTo: '#f472b6',
		category: 'design-tools' as const,
		featured: true
	},
	{
		slug: 'githerald',
		name: 'GitHerald',
		iconPath: 'M10.34 15.84c-.688-.06-1.386-.09-2.09-.09H7.5a4.5 4.5 0 110-9h.75c.704 0 1.402-.03 2.09-.09m0 9.18c.253.962.584 1.892.985 2.783.247.55.06 1.21-.463 1.511l-.657.38a.75.75 0 01-1.021-.27 18.634 18.634 0 01-1.014-2.238.823.823 0 01.55-1.06c.53-.152 1.05-.328 1.57-.527m0-9.18a8.344 8.344 0 00-.985-2.783.75.75 0 01.464-1.511l.656-.38a.75.75 0 011.022.27 18.634 18.634 0 011.014 2.238.823.823 0 01-.55 1.06 12.11 12.11 0 00-1.571.527M12 18.75v.008',
		gradientFrom: '#6366f1',
		gradientTo: '#818cf8',
		category: 'developer-tools' as const,
		featured: false
	},
	{
		slug: 'dictaro',
		name: 'Dictaro',
		iconPath: 'M12 18.75a6 6 0 006-6v-1.5m-6 7.5a6 6 0 01-6-6v-1.5m6 7.5v3.75m-3.75 0h7.5M12 15.75a3 3 0 01-3-3V4.5a3 3 0 116 0v8.25a3 3 0 01-3 3z',
		gradientFrom: '#f59e0b',
		gradientTo: '#fbbf24',
		category: 'productivity' as const,
		featured: false
	},
	{
		slug: 'specwatch',
		name: 'Specwatch',
		iconPath: 'M9.75 3.104v5.714a2.25 2.25 0 01-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 014.5 0m0 0v5.714c0 .597.237 1.17.659 1.591L19.8 15.3M14.25 3.104c.251.023.501.05.75.082M19.8 15.3l-1.57.393A9.065 9.065 0 0112 15a9.065 9.065 0 00-6.23.693L5 14.5m14.8.8l1.402 1.402c1.232 1.232.65 3.318-1.067 3.611A48.309 48.309 0 0112 21c-2.773 0-5.491-.235-8.135-.687-1.718-.293-2.3-2.379-1.067-3.61L5 14.5',
		gradientFrom: '#10b981',
		gradientTo: '#34d399',
		category: 'testing' as const,
		featured: true
	},
	{
		slug: 'apiflip',
		name: 'Apiflip',
		iconPath: 'M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5',
		gradientFrom: '#0ea5e9',
		gradientTo: '#38bdf8',
		category: 'developer-tools' as const,
		featured: false
	},
	{
		slug: 'plainbase',
		name: 'Plainbase',
		iconPath: 'M20.25 6.375c0 2.278-3.694 4.125-8.25 4.125S3.75 8.653 3.75 6.375m16.5 0c0-2.278-3.694-4.125-8.25-4.125S3.75 4.097 3.75 6.375m16.5 0v11.25c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125V6.375m16.5 0v3.75m-16.5-3.75v3.75m16.5 0v3.75C20.25 16.153 16.556 18 12 18s-8.25-1.847-8.25-4.125v-3.75m16.5 0c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125',
		gradientFrom: '#3b82f6',
		gradientTo: '#60a5fa',
		category: 'data-tools' as const,
		featured: false
	},
	{
		slug: 'inglenook',
		name: 'Inglenook',
		iconPath: 'M9 17.25v1.007a3 3 0 01-.879 2.122L7.5 21h9l-.621-.621A3 3 0 0115 18.257V17.25m6-12V15a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 15V5.25m18 0A2.25 2.25 0 0018.75 3H5.25A2.25 2.25 0 003 5.25m18 0V12a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 12V5.25',
		gradientFrom: '#64748b',
		gradientTo: '#94a3b8',
		category: 'ai-infrastructure' as const,
		featured: false
	},
	{
		slug: 'docpeel',
		name: 'Docpeel',
		iconPath: 'M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z',
		gradientFrom: '#14b8a6',
		gradientTo: '#2dd4bf',
		category: 'data-tools' as const,
		featured: false
	},
	{
		slug: 'selfbloom',
		name: 'Selfbloom',
		iconPath: 'M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z',
		gradientFrom: '#e11d48',
		gradientTo: '#fb7185',
		category: 'wellness' as const,
		featured: false
	},
	{
		slug: 'ourobuild',
		name: 'Ourobuild',
		iconPath: 'M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182m0-4.991v4.99',
		gradientFrom: '#7c3aed',
		gradientTo: '#a78bfa',
		category: 'developer-tools' as const,
		featured: false
	},
	{
		slug: 'dataferry',
		name: 'Dataferry',
		iconPath: 'M7.5 21 3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5',
		gradientFrom: '#0891b2',
		gradientTo: '#22d3ee',
		category: 'data-tools' as const,
		featured: false
	},
	{
		slug: 'cohesa',
		name: 'Cohesa',
		iconPath: 'M2.25 12.75V12A2.25 2.25 0 0 1 4.5 9.75h15A2.25 2.25 0 0 1 21.75 12v.75m-8.69-6.44-2.12-2.12a1.5 1.5 0 0 0-1.061-.44H4.5A2.25 2.25 0 0 0 2.25 6v12a2.25 2.25 0 0 0 2.25 2.25h15A2.25 2.25 0 0 0 21.75 18V9a2.25 2.25 0 0 0-2.25-2.25h-5.379a1.5 1.5 0 0 1-1.06-.44Z',
		gradientFrom: '#0d9488',
		gradientTo: '#5eead4',
		category: 'productivity' as const,
		featured: false
	},
	{
		slug: 'wisdone',
		name: 'Wisdone',
		iconPath: 'M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z',
		gradientFrom: '#9333ea',
		gradientTo: '#c084fc',
		category: 'productivity' as const,
		featured: false
	},
	{
		slug: 'knowmine',
		name: 'Knowmine',
		iconPath: 'm21 7.5-9-5.25L3 7.5m18 0-9 5.25m9-5.25v9l-9 5.25M3 7.5l9 5.25M3 7.5v9l9 5.25m0-9v9',
		gradientFrom: '#ca8a04',
		gradientTo: '#facc15',
		category: 'productivity' as const,
		featured: false
	},
	{
		slug: 'shiptron',
		name: 'Shiptron',
		iconPath: 'M9.813 15.904 9 18.75l-.813-2.846a4.5 4.5 0 0 0-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 0 0 3.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 0 0 3.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 0 0-3.09 3.09ZM18.259 8.715 18 9.75l-.259-1.035a3.375 3.375 0 0 0-2.456-2.456L14.25 6l1.035-.259a3.375 3.375 0 0 0 2.456-2.456L18 2.25l.259 1.035a3.375 3.375 0 0 0 2.456 2.456L21.75 6l-1.035.259a3.375 3.375 0 0 0-2.456 2.456Z',
		gradientFrom: '#4338ca',
		gradientTo: '#818cf8',
		category: 'ai-infrastructure' as const,
		featured: false
	},
	{
		slug: 'augur',
		name: 'Augur',
		iconPath: 'M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z',
		gradientFrom: '#7c3aed',
		gradientTo: '#c4b5fd',
		category: 'developer-tools' as const,
		featured: false
	},
	{
		slug: 'agentry',
		name: 'Agentry',
		iconPath: 'M12 6.042A8.967 8.967 0 0 0 6 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 0 1 6 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 0 1 6-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0 0 18 18a8.967 8.967 0 0 0-6 2.292m0-14.25v14.25',
		gradientFrom: '#0d9488',
		gradientTo: '#5eead4',
		category: 'developer-tools' as const,
		featured: false
	},
	{
		slug: 'brainstash',
		name: 'Brainstash',
		iconPath: 'M12 18v-5.25m0 0a6.01 6.01 0 0 0 1.5-.189m-1.5.189a6.01 6.01 0 0 1-1.5-.189m3.75 7.478a12.06 12.06 0 0 1-4.5 0m3.75 2.383a14.406 14.406 0 0 1-3 0M14.25 18v-.192c0-.983.658-1.823 1.508-2.316a7.5 7.5 0 1 0-7.517 0c.85.493 1.509 1.333 1.509 2.316V18',
		gradientFrom: '#db2777',
		gradientTo: '#f9a8d4',
		category: 'productivity' as const,
		featured: false
	},
	{
		slug: 'gitdex',
		name: 'Gitdex',
		iconPath: 'M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5',
		gradientFrom: '#475569',
		gradientTo: '#94a3b8',
		category: 'developer-tools' as const,
		featured: false
	},
	{
		slug: 'daylog',
		name: 'Daylog',
		iconPath: 'M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5',
		gradientFrom: '#0d9488',
		gradientTo: '#5eead4',
		category: 'wellness' as const,
		featured: false
	},
	{
		slug: 'contactly',
		name: 'Contactly',
		iconPath: 'M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z',
		gradientFrom: '#0284c7',
		gradientTo: '#38bdf8',
		category: 'productivity' as const,
		featured: false
	},
	{
		slug: 'linkstash',
		name: 'Linkstash',
		iconPath: 'M13.19 8.688a4.5 4.5 0 0 1 1.242 7.244l-4.5 4.5a4.5 4.5 0 0 1-6.364-6.364l1.757-1.757m13.35-.622 1.757-1.757a4.5 4.5 0 0 0-6.364-6.364l-4.5 4.5a4.5 4.5 0 0 0 1.242 7.244',
		gradientFrom: '#16a34a',
		gradientTo: '#4ade80',
		category: 'productivity' as const,
		featured: false
	},
	{
		slug: 'infra-hub',
		name: 'Infra Hub',
		iconPath: 'M5.25 14.25h13.5m-13.5 0a3 3 0 0 1-3-3m3 3a3 3 0 1 0 0 6h13.5a3 3 0 1 0 0-6m-16.5-3a3 3 0 0 1 3-3h13.5a3 3 0 0 1 3 3m-19.5 0a4.5 4.5 0 0 1 .9-2.7L5.737 5.1a3.375 3.375 0 0 1 2.7-1.35h7.126c1.062 0 2.062.5 2.7 1.35l2.587 3.45a4.5 4.5 0 0 1 .9 2.7m0 0a3 3 0 0 1-3 3m0 3h.008v.008h-.008v-.008Zm0-6h.008v.008h-.008v-.008Zm-3 6h.008v.008h-.008v-.008Zm0-6h.008v.008h-.008v-.008Z',
		gradientFrom: '#0891b2',
		gradientTo: '#67e8f9',
		category: 'ai-infrastructure' as const,
		featured: false
	},
	{
		slug: 'proomptal',
		name: 'Proomptal',
		iconPath: 'M14.25 9.75 16.5 12l-2.25 2.25m-4.5 0L7.5 12l2.25-2.25M6 20.25h12A2.25 2.25 0 0 0 20.25 18V6A2.25 2.25 0 0 0 18 3.75H6A2.25 2.25 0 0 0 3.75 6v12A2.25 2.25 0 0 0 6 20.25Z',
		gradientFrom: '#0ea5e9',
		gradientTo: '#7dd3fc',
		category: 'ai-infrastructure' as const,
		featured: false
	}
] as const;
